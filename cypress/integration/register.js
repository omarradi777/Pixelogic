...
describe("Registration tests", () => {
    ...
    it("Register as a user using API requests and verify registration by logging in to your account", function() {
        /* Visiting register page and filling out register form to sign up */
        cy.request({
            method: 'POST',
            url: 'https://www.phptravels.net/account/signup',
            body: "firstname="+firstName+"&lastname="+lastName+"&phone="+mobileNumber+"&email="+email+".com&password="+password+"&confirmpassword="+password
        }).then((response)=>{
            var x = response.body
        })  
        /* Check data are shown in the account page*/
